...
namespace SIGEFA.Formularios
{
    public partial class frmIngresoCamaras : DevComponents.DotNetBar.OfficeForm
    {
                ...
                else if (Proceso == 2)
                {
                    if (Admingre.update(ingreso))
                    {
                        ...
                        if (detalle.Count > 0)
                        {
                            foreach (clsDetalleIngresoCamara det in detalle)
                            {
                                Admingre.updateDetalleContenedor(det);
                            }
                        }

                        MessageBox.Show("Los datos se actualizaron correctamente", "Ingreso Camara", MessageBoxButtons.OK, MessageBoxIcon.Information);
                        CodIngresoCamara = ingreso.CodIngresoCamara;
                        Proceso = 0;
                        CargaIngresoCamara();
                        sololectura();
        ...
        private void CargaIngresoCamara()
        {
            try
            {
                ...
                if (ingreso != null)
                {
                    ...
                    foreach (DataGridViewRow row in dgvDetalle.Rows)
                        codIngMatPrima = Convert.ToInt32(row.Cells[codproducto.Name].Value);
        ...
        {
            ext.modoeditable(groupBox1.Controls);
            ext.modoeditable(groupBox2.Controls);
            ext.modoeditable(groupBox3.Controls);
            ...
            cmbProcedencia.Enabled = true;
            cboPesador.Enabled = true;
