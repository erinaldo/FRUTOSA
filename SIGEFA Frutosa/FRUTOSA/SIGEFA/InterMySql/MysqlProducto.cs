...
namespace SIGEFA.InterMySql
{
    class MysqlProducto:IProducto
    {
                ...
                if (dr.HasRows)
                {
                    while (dr.Read())
                    {
                        if (Caso == 1)
                        {
                            ...
                            pro.CodFamilia = dr.GetInt32(11);//RAGA
                        }
                        else
                        {
                            pro = new clsProducto();
                            ...
                            pro.CodFamilia = dr.GetInt32(20);//RAGA
                        }
                    }
                }
                return pro;
      ...
      //  ListaTotalIngProdTerm
        public DataTable ListaTotalIngProdTerm(Int32 codTran, Int32 codalmacen, DateTime finicio,DateTime ffin)
        {
            try
            {
                tabla = new DataTable();
                con.conectarBD();
                cmd = new MySqlCommand("ListaTotalIngProdTerm", con.conector);
                cmd.CommandType = CommandType.StoredProcedure;
                cmd.Parameters.AddWithValue("codtran", codTran);
                cmd.Parameters.AddWithValue("codalm", codalmacen);
        ...
        public DataTable ListaTotalSalProd(Int32 codtipoart, Int32 codalmacen, DateTime finicio, DateTime ffin)
        {
            try
            {
                tabla = new DataTable();
                con.conectarBD();
                cmd = new MySqlCommand("ListaTotalSalProduccion", con.conector);
                cmd.CommandType = CommandType.StoredProcedure;
                cmd.Parameters.AddWithValue("codtipoart", codtipoart);
                cmd.Parameters.AddWithValue("codalm", codalmacen);
                cmd.Parameters.AddWithValue("fechaini", finicio);
                cmd.Parameters.AddWithValue("fechafin", ffin);
                adap = new MySqlDataAdapter(cmd);
